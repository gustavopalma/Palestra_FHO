...
class NRF24L01:
    def __init__(self, spi, cs, ce, channel=76, payload_size=16):
        ...
        self.set_power_speed(POWER_0,SPEED_2M)
    ...
    def set_PA_level(self):
        setup = self.reg_read(RF_SETUP) & (0xF8);
        setup |= 3
        print("pa setup",setup)
        self.reg_write(RF_SETUP, setup)
    
    def enable_auto_ack():
       self.reg_write(EN_AA, 0x3F)
...
client_id = 'pico'
lat_topic = 'tracker/lat'
lng_topic = 'tracker/lng'
vel_topic = 'tracker/vel'
...
myPins = {"spi": 0, "miso": 16, "mosi": 19, "sck": 18, "csn": 17, "ce": 21}
# Addresses (little endian)
pipes = (b"\xff\xff\xff\xff\xaa", b"\xff\xff\xff\xff\x55")
print("NRF24L01 Receiver")
csn = Pin(myPins["csn"], mode=Pin.OUT, value=1)
ce = Pin(myPins["ce"], mode=Pin.OUT, value=0)

nrf = NRF24L01(SPI(myPins["spi"]), csn, ce,channel=76, payload_size=32)
...
while True:
    if nrf.any():
        (response,) = struct.unpack("32s", nrf.recv())
        data = response.decode("utf-8") 
        if ( "LAT" in data ):
            print("LAT")
            client.publish(lat_topic, data)
        elif ( "LNG" in data ):
            client.publish(lng_topic, data)
        elif ( "VEL" in data ):
            client.publish(vel_topic, data)
        nrf.flush_rx()
    led.toggle()     
    utime.sleep_ms(50)
